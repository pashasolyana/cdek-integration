            ...
            <div class="fp-container">
                ...
                <header class="fp-header">
                    ...
                    <p class="fp-subtitle" v-if="step === 1">
                        Восстановите пароль и создавайте <br /> заказы как компания
                    </p>
                ...
                <form v-if="step === 1" class="fp-form" novalidate @submit.prevent="onNext">
                    <div class="form-field">
                        ...
                        <input class="form-input" v-model="phone" type="tel" inputmode="tel"
                            placeholder="+7(999)999-99-99" :class="inputClass(phoneValid, 'phone')"
                            :aria-invalid="t.phone && !phoneValid" @blur="touch('phone')" />
                ...
                <form v-else class="rv-form" novalidate @submit.prevent="onVerify">
                    ...
                    <div class="code-inputs" :class="{ invalid: isInvalid }">
                        <input v-for="(_, i) in code" :key="i" :ref="el => (codeRefs[i] = el as HTMLInputElement)"
                            class="code-box" inputmode="numeric" maxlength="1" autocomplete="one-time-code"
                            :aria-label="`Цифра ${i + 1}`" v-model="code[i]" @input="onDigitInput(i, $event)"
                            @keydown.backspace.prevent="onBackspace(i)" @paste.prevent="onPaste" />
                    </div>
...
function goStep(s: 1 | 2) { step.value = s }
...
async function onVerify() {
    ...
    try {
        ...
        const msg = err?.response?.data?.message
        serverError.value = Array.isArray(msg) ? msg.join(', ') : (msg || 'Неверный код или истёк срок действия')
    } finally {
        loading.value = false
    }
}
...
.fp-modal {
    ...
    border-radius: 28px;
    box-shadow:
        0 24px 60px rgba(15, 43, 81, .10),
        0 12px 24px rgba(15, 43, 81, .05);
    display: flex;
    justify-content: center;
}
