...
export class RegionsQueryDto {
  @ApiPropertyOptional({ description: 'Список ISO2-кодов стран через запятую', example: 'RU,KZ' })
  @IsOptional() @IsString() country_codes?: string;

  /** deprecated, но CDEK ещё принимает */
  @ApiPropertyOptional({ description: 'FIAS региона (устаревшее)' })
  @IsOptional() @IsUUID() fias_region_guid?: string;
...
export class CitiesQueryDto {
  @ApiPropertyOptional({ description: 'ISO2 коды стран через запятую', example: 'RU,KZ' })
  @IsOptional() @IsString() country_codes?: string;

  @ApiPropertyOptional({ description: 'Код региона CDEK', example: 78 })
  @IsOptional() @IsInt() @Min(1) region_code?: number;

  @ApiPropertyOptional({ description: 'КЛАДР региона' })
  @IsOptional() @IsString() kladr_region_code?: string;
  ...
  @IsOptional() @IsString() kladr_code?: string;

  @ApiPropertyOptional({ description: 'FIAS города' })
  @IsOptional() @IsUUID() fias_guid?: string;
  ...
  @IsOptional() @IsInt() @Min(1) code?: number;

  @ApiPropertyOptional({ description: 'Название города (полное соответствие)' })
  @IsOptional() @IsString() city?: string;

  @ApiPropertyOptional({ description: 'Лимит НП оплаты (-1 нет ограничений, 0 — не принимается)', example: -1 })
  @IsOptional() @IsNumber() payment_limit?: number;
